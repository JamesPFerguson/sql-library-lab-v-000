...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series INNER JOIN characters ON characters.series_id = series.id WHERE character.species = 'human' GROUP BY series.title ORDER BY COUNT(character.species) DESC LIMIT 1;"
end
